...
- 👁️ Crop disease diagnosis (Vision AI)
...
### High-Level Flow
1. **📡 Sensor Producer (`producer_sensor.py`)**
...
2. **🧠 Analyzer Service (`analyzer.py`) — The Brain**
...
3. **🚨 Notification Service (`notification_consumer.py`)**
...
4. **🌐 API Gateway / Backend Server (`server.py`)**
   - Flask server acting as the bridge to the **React Frontend**.
   - Exposes endpoints for:
     - **Vision AI Image Upload** (`/upload-image`)
     - **Settings sync** (threshold changes from frontend → backend stream/config)

5. **🖥️ Frontend Dashboard (React + TypeScript)**
...
## 🧠 Hybrid Decision Engine (Dual-Brain)
...
Example rules:
- Soil moisture < 60% → Irrigation needed  
- Nitrogen < 50 mg/kg → Fertilization needed  
- Temperature < 21°C or > 28°C OR Humidity > 80% → Energy intervention needed  

### 🤖 ML Engine (Probabilistic)
...
## 👁️ GreenField Vision AI (Deep Learning Module)
...
3. Image is preprocessed (224×224 + normalization).
...
### Safety Logic
- Confidence < 35% → “Uncertain Analysis”
...
│ ├── data_loader.py
│ ├── strategies_model.py
│ ├── strategies_vision.py
│ ├── train_agri_model.py
│ ├── greenfield_agri_brain.h5
│ ├── class_indices.json
│ └── docker-compose.yml
│
├── frontend/
...
### 2️⃣ (Optional) Start Fake SMTP for Local Email Testing
```bash
python debug_server.py
```

### 3️⃣ Start Analyzer Service (Core Brain)
```bash
python analyzer.py
```

### 4️⃣ Start Notification Consumer (Email Alerts)
```bash
python notification_consumer.py
```

### 5️⃣ Start Backend API Gateway (Flask)
```bash
python server.py
```

### 6️⃣ Start Sensor Producer
```bash
...
### 8️⃣ Start Auth Microservice (Profilation)
```bash
...
## Final Note
- GreenField Advisor is built with robustness, transparency, and safety in mind — ensuring that AI enhances human decision-making without replacing agronomic expertise.
